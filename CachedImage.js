...
const { Image, ActivityIndicator, NetInfo, Platform } = ReactNative;
...
const CachedImage = React.createClass({
  ...
  processSource(source) {
    ...
    if (ImageCacheProvider.isCacheable(url)) {
      ...
      options.progressCallback = progress =>
        this.safeSetState({ loading: progress < 1, progress });
  ...
  renderLoader() {
    ...
    if (
      ...
      return this.renderIndicator(activityIndicatorProps, [
        imageStyle,
  ...
  renderIndicator(activityIndicatorProps, activityIndicatorStyle) {
    ...
    let { progress, loading } = this.state;
